...
public:
    ...
    using patients_queue_type = std::map<doctor_type, single_queue>;

    /// @brief Store all the doctors
    using doctors = std::map<doctor_type, std::vector<position>>;
...
private:
    ...
    void insert_in_order(const doctor_type& type, const patient_turn& turn);

    /// @brief Remove an agent from a queue
    /// @param type The doctor type to dequeue from
    /// @param id The agent id
    void remove_patient(const doctor_type& type, const agent_id& id);
};
